...
from utils import load_npy_from_gcs, plot_losses, save_final_loss_plot
...
def train_step(params, model, ims, t, key, alpha_bar, train=True):
    def loss_fn(params):
        ...
        noised = jnp.sqrt(alpha_bar[t])[:, None, None, None] * ims + \
                 jnp.sqrt(1 - alpha_bar[t])[:, None, None, None] * eps
        if train:
            eps_pred = model.apply({'params': params}, noised, t, train=True,
                                   rngs={'dropout': key})
        else:
...
def train(cfg):
    ...
    dummy_x = jnp.ones((1, cfg['data']['image_size'], cfg['data']['image_size'], cfg['model']['in_ch']))
