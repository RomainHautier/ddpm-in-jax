...
# --ladder (second pass, user 2026-08-17): grade uni-final and the best-probe checkpoint
...
DIR = 'monitoring/ddpo_multiregime_unified_gt_ckpts'
...
CK = DIR + '/ddpo_multiuni_iter{:04d}.pkl'
MODELS = {'uni-final': (CK.format(hist[-1]['iter']), 'params'),
          'uni-ema': (CK.format(hist[-1]['iter']), 'ema_params')}
if best_iter != hist[-1]['iter']:
    MODELS[f'uni-{best_iter}'] = (CK.format(best_iter), 'params')
...
if mid_iter not in (best_iter, hist[-1]['iter']):            # behaviour formed early or late?
    MODELS[f'uni-mid{mid_iter}'] = (CK.format(mid_iter), 'params')
...
if LADDER_MODE:
    ...
    LMODELS = ['uni-final'] + [nm for nm in MODELS if nm not in ('uni-final', 'uni-ema')
