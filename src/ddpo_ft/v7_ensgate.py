...
for R, (gt_path, seqs, per, store_path, models) in CFG.items():
    ...
    if all(f'{R}|{m}|K3|v7ensgate||ret' in S for m in models) and not os.environ.get('GATE_FORCE'):
        continue
    ...
    # ABLATION: per-band gates kept, per-sample conditioning REMOVED - every sample
    # is gated against the ENSEMBLE band target (scale identically 1).
    SCALES = {'v7ensgate': np.ones_like(obs, np.float32)}
    _sc = SCALES['v7ensgate']
