...
# GATE_STARTS='125' etc: run the SAME gated guidance on an adapted single chain (the
# dechain configs). The chain token in every store key and field name follows, so
# these rows land beside the MO chain rows (K125|...) instead of overwriting K3.
if os.environ.get('GATE_STARTS'):
    STARTS = [int(x) for x in os.environ['GATE_STARTS'].split(',')]
CHTOK = 'K3' if STARTS == [150, 100, 50] else 'K' + '-'.join(str(t) for t in STARTS)
...
TAG0 = os.environ.get('GATE_TAG', 'v8lowband') + (f"db{int(float(os.environ['GATE_DEADBAND']) * 100)}"
                      if os.environ.get('GATE_DEADBAND') else '')
...
if os.environ.get('GATE_OOD_SEQS'):
    ...
    for _r in list(_ALL):
        if _r >= 1500 or os.environ.get('GATE_SEQS_ALL'):
...
for R, (gt_path, seqs, per, store_path, models) in CFG.items():
    ...
    if all(f'{R}|{m}|{CHTOK}|{TAG0}||ret' in S for m in models) and not os.environ.get('GATE_FORCE'):
        continue
    ...
    for m in models:
        for tag, scale in SCALES.items():
            ...
            for i in range(0, len(recon), 64):
                ...
                def dx(x, _tm=_t_mid, _th=_t_hi, _tl=_t_low):
                    S_ = spec_fn(x)
            ...
            if os.environ.get('SAVE_FIELDS'):     # store reconstructions so per-band placement
                ...
                np.savez_compressed(f'{fd}/{m}__{CHTOK}__{tag}.npz', x=y.astype(np.float16))
                print(f"    saved fields -> {fd}/{m}__{CHTOK}__{tag}.npz", flush=True)
            ...
            np.savez_compressed(f"{_fd}/{m}__{CHTOK}__{tag}.npz", x=np.asarray(y, np.float16))
