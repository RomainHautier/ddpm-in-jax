"""COMBINED: TAPERED EDGE + PER-SAMPLE SCALING (v5, 2026-08-26). The isolated runs showed the
...
for R, (gt_path, seqs, per, store_path, models) in CFG.items():
    ...
    SCALES = {'v5taperscale': (obs / C_R).astype(np.float32)}
    _b = (obs / C_R) ** beta
    SCALES['v5taperbeta'] = (_b / _b.mean()).astype(np.float32)
    print(f"    beta (variance-matched) = {beta:.3f}", flush=True)
    _sc = SCALES['v5taperscale']
...
print("\nV5 COMBINED COMPLETE", flush=True)
