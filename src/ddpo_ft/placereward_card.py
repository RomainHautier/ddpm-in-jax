...
into the shared npz under 'pr2k-<iter>' — directly comparable to the re2k-599 card, isolating
...
CKS = sorted(glob.glob('monitoring/ddpo_re2000_placereward_ckpts/ddpo_re1000_iter*.pkl'))
...
dx2k = make_dx_func(n=N, re=2000.0, std=SIG, mean=MEAN)
...
print("=== PHASE 1: realstats checkpoints @ K3, Re=2000 val (statistics only) ===", flush=True)
scores = {}
for p in CKS:
