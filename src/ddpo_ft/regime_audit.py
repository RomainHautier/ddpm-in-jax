"""REGIME AUDIT (generalized from the Re=1000 audit, user 2026-08-25): same full format at any
regime - env AUDIT_RE selects the regime (val sequences 8-19 of the generated pools, held out:
the generated datasets postdate base pretraining and the fine-tunes used seqs 0-7 LR only),
AUDIT_MODELS = "name:path[:key],..." (base0 always included), AUDIT_STRATS = comma list.
Outputs base_results/regime_audit_re{R}.npz and fields/re{R}/. ORIGINAL DOC: RE=1000 FULL AUDIT on sequences NO model ever trained on: 34-39 of the
...
aggregate hi-k placement, MSE, residual ratio, lowk, spectrum. Keys '{R}|{row}||{field}' in
...
if os.environ.get('AUDIT_STRATS'):
    _ks = set(os.environ['AUDIT_STRATS'].split(',')); STRATS = {k: v for k, v in STRATS.items() if k in _ks}
ddpm, base_params, _ = build_base_ddpm(); ab = ddpm.alpha_bar
MODELS = {'base0': base_params}
for _spec in filter(None, os.environ.get('AUDIT_MODELS', '').split(',')):
...
d = np.load(f'base_results/regime_stats_re{R}_measured_train.npz'); stats = {k: d[k] for k in d.files}
...
if not os.path.exists(f'{FDIR}/GT.npz'):
    ...
    np.savez(f'{FDIR}/index.npz', seqs=np.array(SEQS), per=PER, note=f'val seqs 8-19 of gen_fnons_re{R}; middle frame = channel 1; multiply by SIG=4.7988 for physical vorticity')
...
OUT[f'{R}|GT||psEb'] = np.stack([Eg_s[:, lo:hi].sum(1) for lo, hi in BANDS], 1).astype(np.float32)
...
def grade(y, row):
    ...
    OUT[f'{R}|{row}||psEb'] = np.stack([Es[:, lo:hi].sum(1) for lo, hi in BANDS], 1).astype(np.float32)
    OUT[f'{R}|{row}||ps_ret_paired'] = (Es[:, HIK0:96].sum(1) / Eg_s[:, HIK0:96].sum(1)).astype(np.float32)
    OUT[f'{R}|{row}||ps_ret_ens'] = (Es[:, HIK0:96].sum(1) / E_gt[HIK0:96].sum()).astype(np.float32)
    _Eh = local_hik_energy(y[..., 1] * SIG, HIK0, 6.0)
    OUT[f'{R}|{row}||ps_place'] = np.array([np.corrcoef(_Eh[i].ravel(), Ehg[i].ravel())[0, 1]
                                             for i in range(len(y))], np.float32)
    OUT[f'{R}|{row}||ps_mse'] = (np.mean((y[..., 1] - xg[..., 1]) ** 2, axis=(1, 2)) * SIG ** 2).astype(np.float32)
    OUT[f'{R}|{row}||ps_resid'] = np.concatenate([np.asarray(resid_fn(jnp.asarray(y[i:i + 32]))).ravel()
    ...
    for f, vv in vals.items(): OUT[f'{R}|{row}||{f}'] = np.float32(vv)
    OUT[f'{R}|{row}||E'] = E.astype(np.float32)
    OUT[f'{R}|{row}||Eb'] = np.array([E[lo:hi].sum() / E_gt[lo:hi].sum() for lo, hi in BANDS], np.float32)
    OUT[f'{R}|{row}||bp'] = np.array([np.corrcoef(ym[i].ravel(), gt_maps[i].ravel())[0, 1]
                                       for i in range(5)], np.float32)
    np.savez(OUTP, **OUT)
    print(f"  {row:<26} ret={vals['ret']:.3f} place={vals['place']:.3f} mse={vals['mse']:.2f} "
          f"resid={vals['resid_ratio']:.2f} Eb=" + " ".join(f"{x:.2f}" for x in OUT[f'{R}|{row}||Eb']), flush=True)


for row, arr in (('LR', xl), ('recon', recon)):
    if f'{R}|{row}||ret' not in OUT or any(row.startswith(f) for f in FORCE): grade(arr, row)

xpack = np.concatenate([recon, xl], axis=-1)
for nm, P in MODELS.items():
    for sg, (lp, ls, mu, ver) in STRATS.items():
        row = f'{nm}|K3|{sg}'
        if f'{R}|{row}||ret' in OUT and not any(row.startswith(f) for f in FORCE): continue
...
for cfg in ('K2x50', 'K4x110', 'K5x140'):
    ...
    jax.clear_caches()
print(f"REGIME AUDIT {R} COMPLETE", flush=True)
