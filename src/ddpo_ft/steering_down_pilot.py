"""DOWNWARD-TRANSFER STEERING PILOT (user 2026-08-19): the dial in reverse — a hotter-regime
fine-tune transported to a CALMER regime overshoots (re2k-599 reads retention 1.58 at Re=1000,
K3); the spectral-reward gradient is TWO-SIDED (grad of a squared log-ratio), so the same dial
should walk the overshoot DOWN toward the anchor continuously at fixed depth. If it does, doses
are bidirectionally correctable at sampling time and checkpoint choice loses its sting.

Cells: the two Re=2000-targeted checkpoints (599 = stats nominee/hottest, 149 = anchor pick)
transported DOWN to Re=1000 and Re=1500, lam_spec in {0, 0.5, 1, 2, 4}, K3x86, same battery,
same key convention into steering_pilot.npz ('{R}|{nm}|ls{v}').
"""
import os, sys
...
REGIMES = {
    1000: dict(gt='flow-data/kf_2d_re1000_256_40seed.npy', val_seqs=list(range(4, 20)), per=8,
...
MODELS = {'re2k-149': pickle.load(open(R2K.format(149), 'rb'))['params'],
...
for R, c in REGIMES.items():
    ...
    print(f"\n=== Re={R}: DOWNWARD steering sweep @ K3x86 ===", flush=True)
    for ls in LS:
        ...
        for nm, P in MODELS.items():
            ...
            print(f"  {nm:<9} ls={ls:<4g} blind(src)={blind:.3f}  ret={vals['ret']:.3f}  "
                  ...
                  flush=True)
print("\nDOWNWARD STEERING COMPLETE", flush=True)
