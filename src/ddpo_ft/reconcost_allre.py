"""RECON COST ACROSS ALL REGIMES (user 2026-08-23): the base-DDIM recon step's per-band
...
for R, c in REGIMES.items():
    if f'{R}|LRvsGT||bp' in OUT and R not in (5000, 8000):
        continue
    xg, xl = [], []
    for s in c['seqs']:
    ...
    if f'{R}|LRvsGT||bp' not in OUT:
        gt_maps = band_maps(xg[..., 1] * SIG)
        lr_maps = band_maps(xl[..., 1] * SIG)
        rc_maps = band_maps(np.asarray(recon)[..., 1] * SIG)
        for tag, (a, b) in {'LRvsGT': (lr_maps, gt_maps), 'reconvsGT': (rc_maps, gt_maps),
                            'reconvsLR': (rc_maps, lr_maps)}.items():
            OUT[f'{R}|{tag}||bp'] = bp_corr(a, b)
        for tag, arr in (('LR', xl), ('recon', np.asarray(recon))):
            E = np.asarray(spec_fn(jnp.asarray(arr))).mean(0)
            OUT[f'{R}|{tag}||Eb'] = np.array([E[lo:hi].sum() / Eg[lo:hi].sum()
                                              for lo, hi in BANDS], np.float32)
        np.savez(OUTP, **OUT)
        print(f"Re={R}: LRvsGT " + " ".join(f"{x:.3f}" for x in OUT[f'{R}|LRvsGT||bp'])
              + " | reconvsGT " + " ".join(f"{x:.3f}" for x in OUT[f'{R}|reconvsGT||bp']),
              flush=True)
    # ---- dial v2 test at 5000/8000 ----
    if R in (5000, 8000) and f'{R}|re2k-149|P3mu3||ret' not in OUT:
        ...
        IDX3 = [2, 3, 4]                          # [16,32), [32,64), [64,96) - v2 adds the deep band
        ...
        np.savez(OUTP, **OUT)
        print(f"  P3mu3 (3-band dial): ret={vals['ret']:.3f} place={vals['place']:.3f} "
              f"bp " + " ".join(f"{x:.3f}" for x in OUT[f'{key}||bp']), flush=True)
    jax.clear_caches()
print("RECONCOST ALLRE COMPLETE", flush=True)
