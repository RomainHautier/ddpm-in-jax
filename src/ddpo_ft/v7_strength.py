...
GLAM = float(os.environ.get('GATE_LAM', '3'))
...
for R, (gt_path, seqs, per, store_path, models) in CFG.items():
    ...
    _TAG0 = f'v7s{int(GLAM)}'
    if all(f'{R}|{m}|K3|{_TAG0}||ret' in S for m in models) and not os.environ.get('GATE_FORCE'):
        continue
    ...
    _TAG = f'v7s{int(GLAM)}'
    SCALES = {_TAG: (obs / C_R).astype(np.float32)}
    _sc = SCALES[_TAG]
    ...
    for m in models:
        for tag, scale in SCALES.items():
            ...
            for i in range(0, len(recon), 64):
                ...
                dx = jax.jit(dx)
                smp = make_kchain_ddim_sampler(ddpm.unet, ab, STARTS, STEPS, dx, GLAM, temp=0.30)
                xb = recon[sl]
