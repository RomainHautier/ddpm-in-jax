"""MEASURED-TARGET STEERING (Track A per user: NO extrapolated anchor - the steering potential is built from GROUND-TRUTH statistics, so the dial fixed point is truth). Clone of the pilot (user's proposal 2026-08-18): differentiate the retention-
...
LS = [0.0, 2.0, 8.0, 16.0]   # measured-target rerun: trimmed grid
GEN = 'flow-data/generated/gen_fnons_re{}_kf_1024to256_20seq.npy'
DOWN = {1000: dict(gt='flow-data/kf_2d_re1000_256_40seed.npy', val_seqs=list(range(4, 20)),
                   per=8, anchor='base_results/regime_stats_re1000_measured_train.npz'),
        1500: dict(gt='flow-data/generated/gen_fnons_re1500_kf_1024to256_20seq.npy',
                   val_seqs=list(range(8, 20)), per=10,
                   anchor='base_results/regime_stats_re1500_measured_train.npz')}
REGIMES = {2000: dict(gt=GEN.format(2000), anchor='base_results/regime_stats_re2000_measured_train.npz'),
           5000: dict(gt=GEN.format(5000), anchor='base_results/regime_stats_re5000_measured_train.npz'),
...
OUT = {}
OUTP = 'base_results/steering_measured.npz'
...
# ---- downward block: measured-target steering of the hot fine-tune at calmer regimes ----
R2K = 'monitoring/ddpo_re2000_newpool_ckpts/ddpo_re1000_iter{:04d}.pkl'
DMODELS = {'re2k-149': pickle.load(open(R2K.format(149), 'rb'))['params'],
           're2k-599': pickle.load(open(R2K.format(599), 'rb'))['params']}
for R, c in DOWN.items():
    d = np.load(c['anchor'])
    stats = {k: d[k] for k in d.files}
    A = stats['spec_ref']
    dx_pde = make_dx_func(n=N, re=float(R), std=SIG, mean=MEAN)
    dx_anchor = make_anchor_dose_dx(stats)
    xg, xl_val = pool(c['gt'], c['val_seqs'], c['per'], with_gt=True)
    resid_fn = jax.jit(make_residual_loss(n=N, re=float(R), std=SIG, mean=0.0))
    E_gt = np.asarray(spec_fn(jnp.asarray(xg))).mean(0)
    Ehg = local_hik_energy(xg[..., 1] * SIG, HIK0, 6.0)
    rg = float(np.concatenate([np.asarray(resid_fn(jnp.asarray(xg[i:i + 32]))).ravel()
    ...
    rc_val = B16(lambda xb, kk: ddim20(base_params, _sa * xb + _s1 * jax.random.normal(
        jax.random.fold_in(kk, 1), xb.shape)), xl_val, 500)
    print(f"\n=== Re={R}: measured-target DOWNWARD sweep @ K3x86 ===", flush=True)
    for ls in [0.0, 1.0, 4.0, 8.0]:
        dx_comb = (dx_pde if ls == 0.0 else
                   jax.jit(lambda x, _l=ls: dx_pde(x) + (_l / 3.0) * dx_anchor(x)))
        smp = make_kchain_ddim_sampler(ddpm.unet, ab, STARTS, STEPS, dx_comb, 3.0, temp=0.30)
        for nm, P in DMODELS.items():
            key = f'{R}|{nm}|ls{ls:g}'
            if f'{key}||ret' in OUT:
                continue
            y = B16(lambda xb, kk: smp(P, sa3 * xb + s13 * jax.random.normal(
            ...
            vals = dict(ret=E[HIK0:96].sum() / E_gt[HIK0:96].sum(),
                        place=np.corrcoef(Eh.ravel(), Ehg.ravel())[0, 1],
                        lowk=E[1:5].sum() / E_gt[1:5].sum(), kstar=eff_resolution(E, E_gt),
                        resid_ratio=ry / rg, blind_src=float(E[10:96].sum() / A[10:96].sum()),
                        mse=np.mean((y[..., 1] - xg[..., 1]) ** 2) * SIG ** 2)
            ...
            print(f"  {nm:<9} ls={ls:<4g} statmatch={vals['blind_src']:.3f}  ret={vals['ret']:.3f}  "
                  f"place={vals['place']:.3f}  mse={vals['mse']:.2f}", flush=True)
print("\nMEASURED STEERING COMPLETE", flush=True)
