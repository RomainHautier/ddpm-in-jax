"""ONE MODEL, ALL REGIMES, FiLM ARM: regime conditioning through the time-embedding pathway.

Third conditioning arm, motivated by two negative results on the same acceptance test:
- unconditioned mixture training -> one shared dose, retention row 0.80 -> 0.28 at fixed config;
- ENS residual-FIELD conditioning at the input stem (from-zero AND pretrained-adapter) -> rows
  decay in lockstep with the unconditioned reference; the conditioning was ignored for dose.

Diagnosis both times: the dose is a GLOBAL output-amplitude property, and neither pathway gave
the optimizer a short route from "which regime" to "how much energy". This arm places the knob
on that route directly: a scalar regime code log(Re/1000)/log 8 (0 at the training regime, 1 at
Re=8000) embeds through a 2-layer MLP whose OUTPUT LAYER IS ZERO-INITIALISED and adds to the
time embedding - the pathway every residual block already converts to per-channel modulation,
i.e. the exact mechanism the network uses to behave differently per noise level. At start the
model is BIT-identical to the base (adds exact zeros; no extra feature-path ops, so no bfloat16
rounding either). The kwarg is still `condRes`, so the whole ddim_cond_fn plumbing and the
cond-aware grading sampler work unchanged - the "conditioning signal" is simply the code array.

Everything else mirrors the two previous arms exactly (oracle GT anchors, nine-pool rotation,
K2 chain [100,75]x50, temp 2.5, kl 0.01, EMA 0.99, probe battery every 90 iters). ACCEPTANCE
TEST unchanged and user-scoped: the single-config retention row - flat beats decaying.

Usage: python -m src.ddpo_ft.multiregime_film_ft [--smoke] [--gt_anchors]
"""
import os, sys
...
SAVE_DIR = ('monitoring/ddpo_multiregime_film_gt_ckpts' if GT_ANCHORS
            else 'monitoring/ddpo_multiregime_film_ckpts')
os.makedirs(SAVE_DIR, exist_ok=True)


def RECODE(x, visc):
    """The FiLM 'conditioning signal': a (B,) regime code log(Re/1000)/log 8 from the traced
    viscosity — 0.0 at Re=1000, 1.0 at Re=8000. Signature matches every cond_fn call site."""
...
film_init = film_unet.init(jax.random.PRNGKey(7), dummy_x, jnp.array([1], jnp.int32),
                           train=False, condRes=jnp.zeros((1,), jnp.float32))
...
missing = set(base_params) - set(film_params)
assert extra == {'re_emb_hidden', 're_emb_out'} and not missing, \
...
for k in base_params:
    ...
    cs = jax.tree_util.tree_map(lambda a: a.shape, film_params[k])
    assert bs == cs, f"shape mismatch in shared module {k}: {bs} vs {cs}"
    merged[k] = base_params[k]

# ---- BASE-IDENTITY CHECK: zero-init MLP adds exact zeros to the time embedding, and nothing
# else touches the feature path -> identity should hold at DEVICE precision too, unlike the
# ConditionalUnet identity conv. Asserted in float32; both printed.
tx = jax.random.normal(jax.random.PRNGKey(3), (2, 256, 256, 3))
tt = jnp.array([50, 120], jnp.int32)
code = RECODE(tx, jnp.float32(1.0 / 4000))
with jax.default_matmul_precision('float32'):
    eps_plain32 = ddpm.unet.apply({'params': base_params}, tx, tt, train=False)
    eps_film32 = film_unet.apply({'params': merged}, tx, tt, train=False, condRes=code)
dmax32 = float(jnp.abs(eps_plain32 - eps_film32).max())
eps_plain = ddpm.unet.apply({'params': base_params}, tx, tt, train=False)
eps_film = film_unet.apply({'params': merged}, tx, tt, train=False, condRes=code)
dmax_dev = float(jnp.abs(eps_plain - eps_film).max())
print(f"base-identity check: max diff {dmax32:.2e} (float32)  {dmax_dev:.2e} (device default)",
      flush=True)
assert dmax32 < 1e-3, "merged FiLM model does not reproduce the base — zero-init broken"

ddpm.unet = film_unet

print(f"building {len(ORDER)} pools + rewards (smoke={SMOKE}, cond=FiLM log-Re)", flush=True)
POOLS, REWARDS = {}, {}
...
json.dump(dict(regimes=ORDER, per_seq=PER_SEQ, n_outer=N_OUTER, temp=TEMP,
               ...
               conditioning='FiLM: log(Re/1000)/log8 -> zero-init 2-layer MLP -> added to the '
                            'time embedding (FiLMUnet); bit-identical to base at start',
...
print("MULTIREGIME-FILM FINETUNE COMPLETE", flush=True)
