...
Output: base_results/cascade_ckpt_sweep.npz
"""
...
LADDER = [('K4x110', [200, 150, 100, 50], 110), ('K3x86', [150, 100, 50], 86),
          ('K2x50', [100, 75], 50), ('K1-100x20', [100], 20),
          ('K1-75x20', [75], 20), ('K1-50x12', [50], 12)]
    ...
    for cname, starts, steps in LADDER:
        ...
        for mname, P in models:
            ...
            print(f"  {cname:<11} {mname:<6} {r['ret']:>7.3f} {r['ret_sd']:>6.3f} "
                  ...
                  flush=True)
        np.savez('base_results/cascade_ckpt_sweep.npz', keys=np.array(list(OUT)),
